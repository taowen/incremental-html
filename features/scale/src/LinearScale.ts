...
export class LinearScale extends Feature<{ data: number[], ticksCount: number, range?: [number, number], paddingTop?: number, paddingBottom?: number }> {
    ...
    public scale = scaleLinear().domain(extent(this.props.data) as [number, number]).range(this.range);
    ...
    private get range(): [number, number] {
        ...
        return [this.parent.height - (this.props.paddingTop || 0) - (this.props.paddingBottom || 0), 0];
    }

    public get chart() {
        return {
            y: this.props.paddingTop || 0,
            height: this.range[0]
        }
    }
    private get parent() {
    ...
    public get ticks() {
        return this.scale.ticks(this.props.ticksCount)
    }
}